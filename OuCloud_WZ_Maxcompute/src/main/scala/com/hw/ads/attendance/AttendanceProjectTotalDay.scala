 ...
 * @author linzhy
 */
object AttendanceProjectTotalDay extends Logging{
  def main(args: Array[String]) {
    ...
    val sqlstr =
      s"""
        |insert overwrite table ads_gx_attendance_project_total_day
        |select
        |  concat(cl.pid,year(date_add(now(), -1)),month(date_add(now(), -1)),day(date_add(now(), -1))) as id,
        |  cl.pid,
        |  cl.project_name,
