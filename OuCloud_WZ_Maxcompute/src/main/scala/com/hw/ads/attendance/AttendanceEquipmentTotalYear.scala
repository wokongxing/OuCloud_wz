 ...
 * @author linzhy
 */
object AttendanceEquipmentTotalYear extends Logging{
  def main(args: Array[String]) {
    ...
    val sqlstr =
      s"""
        ...
        |  sum(year_status) as sn_online_count,
        ...
        |  group by pe.county_code
        |)
        |insert overwrite table ads_gx_attendance_equipment_total_year
        |  select
        |  concat(area.code,year(date_add(now(), -1))) as id,
        |  area.code,
        |  area.name,
