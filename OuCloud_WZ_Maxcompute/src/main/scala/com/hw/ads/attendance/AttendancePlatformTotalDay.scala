 ...
 * @author linzhy
 */
object AttendancePlatformTotalDay extends Logging{
  def main(args: Array[String]) {
    ...
    val sqlstr =
      s"""
        |insert overwrite table ads_gx_attendance_platform_total_day
        |  select
        |    concat( year(date_add(now(), -1)),month(date_add(now(), -1)),day(date_add(now(), -1)),
        ...
        |  coalesce(cl.project_city_code,cl2.project_city_code) city_code,
        |  coalesce(cl.project_county_code,cl2.project_county_code) county_code,
        ...
        |    sum(case when cl.is_job=1 and to_date(cl.job_date) < to_date(now()) then 1 else 0 end ) onjob_total
        |    from
        ...
        |      where cl.is_job=0
        |      and   year(cl.job_end_date) = year(date_add(now(), -1))
        |      and   month(cl.job_end_date)= month(date_add(now(), -1))
        |      and   day(cl.job_end_date)= day(date_add(now(), -1))
        |      group by cl.nativeplace_id,cl.project_city_code,cl.project_county_code
