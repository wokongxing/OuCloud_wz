 ...
 * @author linzhy
 */
object AttendancePlatformTotalMonth extends Logging{
  def main(args: Array[String]) {
    ...
    val sqlstr =
      s"""
        |insert overwrite table ads_gx_attendance_platform_total_month
        |  select
        |    concat( year(date_add(now(), -1)),month(date_add(now(), -1)),
