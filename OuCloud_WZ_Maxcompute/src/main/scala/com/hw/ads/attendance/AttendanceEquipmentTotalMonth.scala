 ...
 * @author linzhy
 */
object AttendanceEquipmentTotalMonth extends Logging{
  def main(args: Array[String]) {
    ...
    val sqlstr =
      s"""
        ...
        |  sum(month_status) as sn_online_count,
        ...
        |  group by pe.county_code
        |)
        |insert overwrite table ads_gx_attendance_equipment_total_month
        |  select
        |  concat(area.code,year(date_add(now(), -1)),month(date_add(now(), -1))) as id,
        |  area.code,
        |  area.name,
